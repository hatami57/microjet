...
- **Idempotency** — `middleware.Idempotency` replays the stored response for a repeated non-safe request carrying the same `Idempotency-Key`, so client retries don't act twice. Keys are scoped by caller (tenant and JWT subject by default, or `WithIdempotencyScope`), method, and request URI, and a key reused with a different body is rejected with 422; backed by any store satisfying a small Get/Set interface (the app cache fits directly).
